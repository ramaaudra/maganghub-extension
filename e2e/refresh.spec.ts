...
async function routeList(context: import('@playwright/test').BrowserContext): Promise<void> {
  await context.route('https://maganghub.kemnaker.go.id/**', (route) => {
    ...
    return route.fulfill({ status: 200, contentType: 'text/html; charset=utf-8', body: listHtml() });
  });
}
...
async function starAllCards(page: import('@playwright/test').Page): Promise<void> {
...
test('a single-favorite refresh shows the open status badge', async ({
  ...
  const popup = await openPopup(context, extensionId);
  await stageFixtures(popup, { [UUID_OPEN]: { status: 200, body: openHtml() } });
...
test('a failed refresh (non-gone HTTP) shows "refresh gagal" with no data loss', async ({
  ...
  const popup = await openPopup(context, extensionId);
  await stageFixtures(popup, { [UUID_OPEN]: { status: 503, body: '<html><body>503</body></html>' } });

  const card = popup.locator(`[data-favorite-uuid="${UUID_OPEN}"]`);
  await card.getByRole('button', { name: 'Segarkan Status Lowongan' }).click();

  // The refresh failed → "Refresh gagal" badge, and the snapshot is still shown.
  await expect(card.getByText('Refresh gagal', { exact: true }).first()).toBeVisible();
...
test('a removed Lowongan (HTTP 404) shows Tutup', async ({
  ...
  const popup = await openPopup(context, extensionId);
  await stageFixtures(popup, { [UUID_CLOSED]: { status: 404, body: '<html><body>404</body></html>' } });
