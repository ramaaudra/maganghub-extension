...
describe("toLiveStatus", () => {
  ...
  it("replaces live fields on a successful parse", () => {
    const response: OffscreenResponse = {
      ok: true,
      uuid: "u1",
      parsed: { status: "open", kuota: 50, pelamar: 12, batch: "Batch 1 · 2026", tunjangan: "Dari Pemerintah" },
    };
  ...
  it("maps a network/parse failure (no httpStatus) to unknown", () => {
    const response: OffscreenResponse = { ok: false, uuid: "u4", error: "network: timeout" };
...
describe("runWithConcurrency", () => {
  it("runs all items and returns results in input order, throttled to the limit", async () => {
    ...
    expect(results.map((r) => (r as { status: "fulfilled"; value: number }).value)).toEqual([
      10, 20, 30, 40, 50,
    ]);
  });
