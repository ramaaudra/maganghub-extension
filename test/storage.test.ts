...
function makeFavoriteV1(uuid: string, savedAt: string, title: string): FavoriteV1 {
  return {
    schemaVersion: 1,
    uuid,
    detailUrl: `/magang-nasional/lowongan/${title.toLowerCase().replace(/\s+/g, '-')}-${uuid}`,
...
describe('favorites storage', () => {
  ...
  it('createFavorite defaults catatan to empty and statusLamar to not_applied', () => {
    const fav = createFavorite({
      ...
      detailUrl: '/magang-nasional/lowongan/x-55555555-5555-4555-8555-555555555555',
      savedSnapshot: { title: 'T', organizer: 'O', location: 'L', capturedAt: '2026-01-01T00:00:00Z' },
    });
  ...
  it('setCatatan updates the Catatan on a stored favorite', async () => {
    const fav = makeFavorite('66666666-6666-4666-8666-666666666666', '2026-01-01T00:00:00Z', 'Magang C');
    await setFavorite(fav);
    await setCatatan(fav.uuid, 'alasan aku simpan ini');
    expect((await getFavorite(fav.uuid))?.catatan).toBe('alasan aku simpan ini');
  });

  it('setCatatan edits an existing Catatan to a new value', async () => {
    const fav = makeFavorite('a6a6a6a6-a6a6-4a6a-8a6a-a6a6a6a6a6a6', '2026-01-01T00:00:00Z', 'Magang G');
    await setFavorite(fav);
    await setCatatan(fav.uuid, 'catatan pertama');
    await setCatatan(fav.uuid, 'catatan kedua yang lebih panjang');
    expect((await getFavorite(fav.uuid))?.catatan).toBe('catatan kedua yang lebih panjang');
  });

  it('setCatatan can clear a Catatan back to empty (delete)', async () => {
    const fav = makeFavorite('b7b7b7b7-b7b7-4b7b-8b7b-b7b7b7b7b7b7', '2026-01-01T00:00:00Z', 'Magang H');
    await setFavorite(fav);
  ...
  it('setStatusLamar updates Status Lamar on a stored favorite', async () => {
    const fav = makeFavorite('77777777-7777-4777-8777-777777777777', '2026-01-01T00:00:00Z', 'Magang D');
    await setFavorite(fav);
  ...
  it('getFavorite lazily migrates a v1 record to the current schema', async () => {
    const v1 = makeFavoriteV1('88888888-8888-4888-8888-888888888888', '2026-01-01T00:00:00Z', 'Magang E');
    await browser.storage.local.set({ [`${FAVORITE_KEY_PREFIX}${v1.uuid}`]: v1 });
  ...
  it('listFavorites lazily migrates v1 records to the current schema', async () => {
    const v1 = makeFavoriteV1('99999999-9999-4999-8999-999999999999', '2026-01-01T00:00:00Z', 'Magang F');
    await browser.storage.local.set({ [`${FAVORITE_KEY_PREFIX}${v1.uuid}`]: v1 });
