...
function getChromeOffscreen(): ChromeOffscreen | undefined {
  const g = globalThis as unknown as { chrome?: { offscreen?: ChromeOffscreen } };
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (attempt === maxAttempts) {
        return { ok: false, uuid, error: `offscreen unreachable: ${String(err)}` };
      }
    }
...
async function refreshAll(): Promise<void> {
  ...
  await ensureOffscreen();
  await runWithConcurrency(favorites, REFRESH_CONCURRENCY, async (fav, index) => {
    if (index > 0) await delay(REFRESH_STAGGER_MS * (index % REFRESH_CONCURRENCY));
    const previous = (await getFavorite(fav.uuid))?.liveStatus;
    const response = await fetchAndParse(fav.uuid, resolveDetailUrl(fav.detailUrl));
