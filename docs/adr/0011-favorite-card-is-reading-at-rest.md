...
- **The resting card is three lines**, ~76px, so roughly six fit the viewport: (1) title + Status Lamar chip, (2) Penyelenggara · Lokasi, (3) the decision line — seat count, a Catatan mark when a note exists, Status Kuota chip, chevron.
...
- **Two things stay outside the tray** because they are news, not chrome: the **change notice** (a quota count that moved since the last check — the user must not have to open a card to learn the quota is full) and the **two chips** (they *are* the resting-state summary).
...
- Two new pure modules, `src/lib/seats.ts` and `src/lib/titlecase.ts`, unit-tested without a browser (prior art: `filter.ts`, `group.ts`). `seats.ts` shares `NEAR_FULL_THRESHOLD` with the refresh parser for presentation-only seat pressure; the Status Kuota chip itself comes from the counts.
...
- The card's layout remains storage-agnostic. The live status taxonomy and its schema migration are defined separately by ADR-0013; the content script remains untouched.
